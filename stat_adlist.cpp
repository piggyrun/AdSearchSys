...
typedef __gnu_cxx::hash_map<uint64_t, uint32_t> t_uint_uint_map;
...
int main(int argc, char * argv[])
{
	...
	//fstream ftmp(tmp_file.c_str(), fstream::in | fstream::out);
	...
	while (getline(in, line))
	{
		...
		for (size_t i=0; i < adid_list.size(); i++)
		{
			...
			uint64_t key = get_md5_64(pv_sample.c_str());
			if (sample_pv_map.find(key) == sample_pv_map.end())
			{
				ftmp << pv_sample << endl;
	...
	// click record
	t_uint_uint_map sample_click_map;
	t_uint_uint_map sample_price_map;
	...
	while (getline(in, line))
	{
		...
		uint64_t key = get_md5_64(click_sample.c_str());
		if (sample_pv_map.find(key) != sample_pv_map.end())
		{
			sample_click_map[key]++;
		}
		else 
		{
			sample_pv_map[key]++;
			ftmp << click_sample << endl;
			md5_list.push_back(key);
			sample_click_map[key]++;
		}
		// click price
		sample_price_map[key] = atoi(tmp[14].c_str());
	}
	in.close();
	...
	for (size_t i=0; i<md5_list.size(); i++)
	{
		...
		it = sample_pv_map.find(md5_list[i]);
		if (!(getline(ftmp, record_line) && it != sample_pv_map.end()))
		{
		...
		ctr = (float)click/pv;
		out <<  pv << "\t" << click << "\t" << ctr << "\t" << record_line << "\t" << price << endl;
	}
	out.close();
	ftmp.close();
